...
def local_file():
    ...
    def _local_file(name):
        return os.path.join(self_dir, 'wdl', name)
...
def test_hello(run, local_file):
    ...
    task = galp.wdl.run(wdl_uri, infile=infile, pattern="^[a-z]+$")
...
def test_missing_arg(run, local_file):
    ...
    with pytest.raises(galp.TaskFailedError):
        run(galp.wdl.run(wdl_uri, infile=infile)) # No "pattern"
...
def test_two_parts(run, local_file):
    ...
    up_task = galp.wdl.run(up_wdl)
    down_task = galp.wdl.run(down_wdl, **{
